    ...
    if (settings.use_rpc) {
      if (settings.nethash == 'netmhashps') {
        rpcCommand([{method:'getmininginfo', parameters: []}], function(response){
          if (!response || response == 'There was an error. Check your console.' || response == '-') {
            return cb('-');
          }
          if (response.netmhashps && response.netmhashps !== '-') {
    ...
    if (settings.use_rpc) {
      rpcCommand([{method:'getdifficulty', parameters: []}], function(response){
        ...
        if (typeof response === 'object' && (response['proof-of-stake'] !== undefined || response['proof-of-work'] !== undefined)) {
          ...
          // fallback: return as-is (legacy or error)
          return cb(response);
        }
      });
    } else {
      ...
      request({uri: uri, json: true}, function (error, response, body) {
        // Expecting Peercoin-style object from API
        return cb(body);
      });
    }
  },

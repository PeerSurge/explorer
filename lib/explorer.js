...
// ------------------------------------------------------------
// Peercoin RPC Normalization Helpers
// ------------------------------------------------------------
function normalize_tx(tx) {
  if (!tx) return tx;
  // Peercoin: sometimes returns numbers as strings
  if (typeof tx.value === 'string') {
    tx.value = parseFloat(tx.value) || 0;
  }
  // Ensure vin/vout arrays exist
  if (!Array.isArray(tx.vin)) tx.vin = [];
  if (!Array.isArray(tx.vout)) tx.vout = [];
  return tx;
}

// ------------------------------------------------------------
// Peercoin: Detect coinstake transactions
// ------------------------------------------------------------
is_coinstake: function(tx) {
  if (!tx || !Array.isArray(tx.vin) || !Array.isArray(tx.vout)) return false;
  // Peercoin coinstake pattern:
  // vin[0] has no txid
  // vout[1] contains the stake reward
  if (tx.vin.length === 1 &&
      !tx.vin[0].txid &&
      tx.vout.length >= 2) {
    return true;
...
// ------------------------------------------------------------
// Peercoin: Normalize vout for PoS
// ------------------------------------------------------------
normalize_pos_vout: function(vout, vin) {
  // If vout[0] is nonstandard, this is likely a PoS marker
  if (vout.length > 1 &&
      vout[0].scriptPubKey &&
      vout[0].scriptPubKey.type === 'nonstandard') {
    // If vin[0] and vout[1] share the same address, adjust reward
    if (vin.length > 0 &&
        vout[1].scriptPubKey &&
        vout[1].scriptPubKey.addresses &&
        vin[0].addresses === vout[1].scriptPubKey.addresses[0]) {
      // Reward = vout[1] - vin[0]
      try {
        const reward = parseFloat(vout[1].value) - parseFloat(vin[0].amount / 100000000);
        vout[1].value = reward < 0 ? 0 : reward;
      } catch (e) {
        // fallback: leave value unchanged
      }
    }
  }
  return vout;
...
module.exports = {
  ...
  get_block: function(hash, cb) {
    if (settings.use_rpc) {
      rpcCommand([{method:'getblock', parameters: [hash]}], function(response){
        // Peercoin: block.reward does not exist
        if (response && response.reward === undefined) {
          response.reward = 0;
        }
        // Peercoin: height may be string
        if (typeof response.height === 'string') {
          response.height = parseInt(response.height);
        }
        // Peercoin: ensure tx array exists
  ...
  get_rawtransaction: function(hash, cb) {
    if (settings.use_rpc) {
      rpcCommand([{method:'getrawtransaction', parameters: [hash, 1]}], function(response){
        // Normalize Peercoin RPC quirks
        response = normalize_tx(response);
        // Detect Peercoin coinstake
        response.isCoinStake = detect_coinstake(response);
        // Extract reward for PoS
        if (response.isCoinStake) {
          try {
            response.reward = parseFloat(response.vout[1].value) || 0;
          } catch (e) {
            response.reward = 0;
          }
        }
      ...
      if ( settings.supply == 'HEAVY' ) {
        ...
        client.command([{method:'gettxoutsetinfo', parameters: []}], function(err, response){
          if(err){console.log('Error: ', err); }
            ...
            // Peercoin: ensure numeric total_amount
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') {
        // ------------------------------------------------------------
        // Peercoin: normalize PoS vout before processing
        // ------------------------------------------------------------
        vout = module.exports.normalize_pos_vout(vout, arr_vin);
  ...
  prepare_vin: function(tx, cb) {
    var arr_vin = [];
    // ------------------------------------------------------------
    // Peercoin: normalize PoS vin before processing
    // ------------------------------------------------------------
    tx = module.exports.normalize_pos_vin(tx);
